...
bool CheckForTransmit(void)
{

bool ReturnVal = false; 
ES_Event CheckerEvent;
	...
	{
		//printf("echk2\n\r"); 
		if (QueryPlayerTransmitSM() != Sleep)
		{
			//printf("echk3\n\r");
			
			CheckerEvent.EventType = TRANSMIT;
			PostPlayerTransmitSM(CheckerEvent); 
			ReturnVal = true;
			//PTU ^= BIT5HI; 
...
ES_Event CheckerEvent;
    ...
    {
        CheckerEvent.EventType = PUSH;
        PostPlayerReceiveSM(CheckerEvent);
        PTT &= BIT3LO; 
        ReturnVal = true; 
